...
window.Sound = (function() {
	...
	function playMusic(file, loop, volumeModifier = 0) {
		try {
			...
			if (musicFile === "") {
				...
				$('#tally_music_source').attr("src", chrome.runtime.getURL("assets/sounds/music/" + musicFile));
				// set params
	...
	// 	// load/play sound
	// 	var audio = new Audio(chrome.runtime.getURL("assets/sounds/" + soundFile));
	// 	audio.muted = true;
	// 	audio.pause();
	...
	function play(soundFile, delay = 0, volumeModifier = 0) {
		try {
			...
			// add source
			$('#tally_audio_source').attr("src", chrome.runtime.getURL("assets/sounds/" + soundFile));
